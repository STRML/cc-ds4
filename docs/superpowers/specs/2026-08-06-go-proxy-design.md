...
- **JSON re-serialization replicates Python's `json.dumps` byte-for-byte.** Python always re-serializes the entire payload with `json.dumps(payload)` (proxy.py:836) using default separators (`', '` / `': '`) and `ensure_ascii=True`. Go must produce the **same bytes** for the rewritten payload. Two Go defaults must be overridden or parity breaks on real requests:
  - **`SetEscapeHTML(false)`** — Python does NOT escape `<`, `>`, `&`; Go's default escapes them as `<`/`>`/`&`. Any request containing those characters would diverge.
  ...
  Parse with order preservation (a JSON order-preserving decoder — `json.Decoder` with token-stream re-emission, not `map[string]any`), apply the rewrite, serialize with Python-matching separators/escaping, and round-trip numbers via `json.Number`. The corpus adds an **HTML-char case** and an **integral-float/big-int case**. **Byte-slice mutation (sjson/jsonparser) is wrong here** — it preserves client whitespace/escaping where Python emits normalized `json.dumps` bytes, breaking the raw-byte parity the gate asserts.
