...
- **JSON re-serialization replicates Python's `json.dumps` byte-for-byte.** Python always re-serializes the entire payload with `json.dumps(payload)` (proxy.py:836) using default separators (`', '` / `': '`) and `ensure_ascii=True`. Go must produce the **same bytes** for the rewritten payload. Three Go defaults diverge from Python and must be overridden or parity breaks on real requests:
  - **Non-ASCII escaping (the big one):** Python's `ensure_ascii=True` escapes every non-ASCII rune to `\uXXXX` (astral as surrogate pairs: 😀 → `😀`); Go's `encoding/json` emits raw UTF-8 and has **no option to escape non-ASCII**. Replicating Python requires a **custom string escaper** (walk each string, escape runes > U+007E to `\uXXXX`, astral as surrogate pairs) applied to every string the serializer emits. This diverges on ordinary Claude Code requests — em-dashes, curly quotes, CJK, emoji — so it is load-bearing, not cosmetic.
