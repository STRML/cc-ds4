...
A prior diagnosis found proxy-side defects that a Go rewrite fixes by construction: unbounded thread-per-request (GIL contention, no cap), and a classifier `urlopen` with **no timeout** (a hung classifier blocks a thread with no deadline).
...
- **The idle timeout is a custom `net.Listener`, NOT `http.Server.ReadTimeout`/`WriteTimeout`.** Go's `ReadTimeout`/`WriteTimeout` are **absolute durations** that sever an SSE stream at the deadline regardless of activity — Python's `socket.settimeout()` is an **idle** timeout (time between bytes). A 60s absolute WriteTimeout truncates a multi-minute SSE stream. The Go proxy wraps each `net.Conn` and resets `SetReadDeadline()`/`SetWriteDeadline()` **on each successful `Read()`/`Write()`** (the deadline is refreshed to `now + RELAY_TIMEOUT` after every operation that makes progress, so the clock only runs while the stream is idle). When `DS4_RELAY_TIMEOUT=0`, the wrapper does not set deadlines (disabled mode = no deadline). (An absolute ceiling is a Phase B item — antigravity's slowloris point.)
...
- **JSON serialization preserves order.** Go must NOT unmarshal the request into `map[string]any` (which alphabetically reorders keys on marshal) and re-serialize. Use a byte-slice mutation approach (e.g. `tidwall/sjson`/`buger/jsonparser`) to rewrite only `messages`/`model`/`max_tokens`/`thinking`, leaving surrounding raw bytes as the client sent them. `UseNumber`-equivalent for numeric fidelity (Python preserves big ints; float64 mangles `9007199254740993`).
...
- Fix the known path bug separately: `ds4-effort:16` maps `openrouter` → `~/.claude-or-ds4`, not `~/.claude-openrouter`.
