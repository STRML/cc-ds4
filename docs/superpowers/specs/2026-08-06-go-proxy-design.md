...
- **The relay idle-timeout is on the UPSTREAM connection, mirroring Python.** Python's `RELAY_TIMEOUT` is a `socket.settimeout()` on the upstream `urlopen` (proxy.py:868) whose documented purpose is the stalled-origin case ("reads that produce no data for this long count as a failure", proxy.py:92-98). Go must apply the same idle deadline to the **upstream** connection — e.g. a per-`DialContext` connection wrapper that resets `SetReadDeadline()`/`SetWriteDeadline()` on each successful `Read()`/`Write()`, refreshing to `now + RELAY_TIMEOUT` so the clock only runs while the stream is idle. This is NOT `http.Server.ReadTimeout`/`WriteTimeout` (absolute, would sever SSE) and NOT a server-side `net.Listener` on the accepted client connection (client-side deadlines never fire while a handler goroutine is blocked reading upstream — that would silently drop the exact behavior `RELAY_TIMEOUT` exists for). When `DS4_RELAY_TIMEOUT=0`, no deadline is set.
- **Stall semantics match Python exactly — there is no clean 502.** `_relay` catches nothing around `_stream` (proxy.py:907), so a timeout surfaces per the phase of the stall: **stall before the first upstream byte** → `urlopen` raises `TimeoutError` with no response object → the proxy emits its 502 "proxy upstream failure"; **stall mid-stream** (headers + some body already relayed) → `TimeoutError` surfaces in the `_stream` read loop → the client already got a partial 200 and the connection dies. A Go implementation must reproduce **both** — a 502 only when nothing has been sent, and a partial-body-then-dead-connection mid-stream. **Go must NOT convert a mid-stream stall into a clean 502** — that would be a parity break. The differential corpus adds both stall cases and asserts they match Python. **Breaker accounting ordering:** Python calls `failover_record` *before* `_stream` (proxy.py:887), so a mid-stream stall is recorded as a breaker **hit** (the 200 landed; the subsequent timeout is invisible to the breaker). Go must record the relay outcome before streaming, exactly as Python does — a Go implementation that records after the stream (counting the stall as a strike) would diverge the breaker state after a stall. (An absolute request ceiling is a Phase B item — antigravity's slowloris point.)
...
- Classifier relay is **one generic helper** `relayClassifier(body, endpoint, auth, headers)` (Python duplicates `_relay_or_ds4`/`_relay_anthropic`; Go does not). **Classifier requests use the request context without a deadline** in Phase A (match Python's no-timeout); ordinary ds4 relay requests use the idle-timeout wrapper. Because the idle deadline lives in a per-`DialContext` wrapper on the `http.Transport`, a classifier request would inherit `RELAY_TIMEOUT` unless it uses a **separate transport client** (or a context-keyed opt-out) — pin that the classifier calls go through a deadline-free client, or the no-timeout parity breaks. Distinct retry/no-retry boundaries preserved: the main relay retries per `ds4-high`/`ds4-xhigh`; classifier and vision child calls do **not** carry the main retry policy.
...
- **JSON re-serialization replicates Python's `json.dumps` byte-for-byte.** Python always re-serializes the entire payload with `json.dumps(payload)` (proxy.py:836) using default separators (`', '` / `': '`) and `ensure_ascii=True`. Go must produce the **same bytes** for the rewritten payload. Three Go defaults diverge from Python and must be overridden or parity breaks on real requests:
  - **Non-ASCII escaping (the big one):** Python's `ensure_ascii=True` escapes every non-ASCII rune to `\uXXXX` (astral as surrogate pairs — the emoji U+1F600 becomes the four literal characters backslash-u-d-8-3-d backslash-u-d-e-0-0, i.e. the two escape sequences `\ud83d` `\ude00`, in the output bytes); Go's `encoding/json` emits raw UTF-8 and has **no option to escape non-ASCII**. Replicating Python requires a **custom string escaper** (walk each string, escape runes > U+007E to `\uXXXX`, astral as surrogate pairs) applied to every string the serializer emits. This diverges on ordinary Claude Code requests — em-dashes, curly quotes, CJK, emoji — so it is load-bearing, not cosmetic. **Exponent/shortest-form floats:** `json.Number` re-emits literals verbatim while Python re-formats through float `repr` — a request containing `1e3` would diverge (Python `1000.0` vs Go `1e3`). Add a corpus case with an exponent-form or non-shortest-form float literal.
  - **`SetEscapeHTML(false)`** — Python does NOT escape `<`, `>`, `&`; Go's default escapes them to `<`/`>`/`&` (Go escapes *to* those `\uXXXX` forms, not to literal chars). Set `SetEscapeHTML(false)` so they pass through unescaped like Python.
  - **`UseNumber` round-tripping** — Python emits `1.0` for an integral float and preserves big integers; Go's float64 coercion mangles both (`1.0`→`1`, `9007199254740993`→`9007199254740992`).
  Parse with order preservation (a JSON order-preserving decoder — `json.Decoder` with token-stream re-emission, **not `map[string]any`** — Go re-marshals a map with alphabetically-sorted keys, which reorders the payload), apply the rewrite, and serialize with Python-matching separators, ASCII escaping, and `json.Number` round-tripping. The corpus adds **non-ASCII (emoji/CJK/em-dash), HTML-char, and integral-float/big-int cases**. **Byte-slice mutation (sjson/jsonparser) is wrong here** — it preserves client whitespace/escaping where Python emits normalized `json.dumps` bytes, breaking the raw-byte parity the gate asserts.
  - **The classifier relay is a second serialization path.** It builds a separate whitelist-filtered body (`classifier.py:63-77,107-110`) serialized with its own `json.dumps` (proxy.py:942,978) — key set and order differ from the main rewrite. It must use the same order-preserving, `ensure_ascii`-equivalent serializer, and the outbound recorder must assert its bytes too.
...
**How both proxies are pointed at the fakes:** `PROFILES` hardcodes the real upstream URLs and `src/proxy.py` has no upstream-override env, and the spec forbids mutating the frozen oracle (Source of truth below). The harness therefore boots the Python proxy **in-process via a test shim** that imports `proxy`, patches `PROFILES` upstreams to the fake URLs, and calls `serve()` — mirroring how the existing test suite already imports and mutates `proxy.PROFILES` (tests/test_proxies.py:659-740). **The shim must also supply a temp profile dir with a credential** — `serve()` sets `require_client_auth=True` (proxy.py:1094), so every POST would 401 without one. The Go proxy is given the same fake upstreams via its build-time-generated config pointing at the fakes. Neither source file is edited; the override lives entirely in the harness.
...
- **upstream stall (both phases, matching Python):** stall before the first upstream byte → 502 "proxy upstream failure" within `RELAY_TIMEOUT`; stall mid-stream (headers + some body already relayed) → partial-200 then the connection dies (no clean 502). Go must reproduce both exactly — it must NOT convert a mid-stream stall into a clean 502
