...
profile's requests are served by the **direct** profile instead — every
`ds4-*` tier maps to `deepseek-v4-flash[1m]` (flash only, no pro — the
fallback must stay cheap), the client's auth header is swapped for the direct
